...
function serverHandler(request, response) {
    try {
        ...
        try {
            ...
            if (filename && filename.search(/apps/g) === -1 && stats.isDirectory()) {
                response.writeHead(200, {
                    'Content-Type': 'text/html'
                });
                response.write('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=/apps/"></head><body></body></html>');
                response.end();
                return;
            }
        } catch (e) {
        ...
        if (fs.statSync(filename).isDirectory()) {
            ...
            // if (filename.indexOf('/apps/MultiRTC/') !== -1) {
            //     filename = filename.replace('/apps/MultiRTC/', '');
            //     filename += '/apps/MultiRTC/index.html';
            // } else 
            if (filename.indexOf('/apps/') !== -1) {
                filename = filename.replace('/apps/', '');
        ...
        fs.readFile(filename, 'utf8', function(err, file) {
            ...
            //                 otherDemos += '<li><a href="/apps/' + f + '">' + f + '</a> (<a href="https://github.com/muaz-khan/RTCMultiConnection/tree/master/demos/' + f + '">Source</a>)</li>';
            //             }
            //         });
